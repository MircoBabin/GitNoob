...
namespace GitNoob.Gui.Program.Remedy
{
    ...
    {
        public MessageLockNotAcquired(Step.Step Step, VisualizerMessageWithLinks Message, Git.GitLockResult result) :
            base(Step, ref Message)
        {
            ...
            if (result.LockedTime != null && result.LockedTime.Value != null)
            {
                ...
                VisualizerMessageText.Append(" - " + GitUtils.DateTimeToHumanString(time));
                VisualizerMessageText.Append(Environment.NewLine);
            }
